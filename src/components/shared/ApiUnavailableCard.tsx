...
export const ApiUnavailableCard: React.FC<ApiUnavailableCardProps> = ({
  title = "Live Operational Data Unavailable",
  description = "Could not establish connection to the FastAPI backend service. Live metrics, queue forecasts, and optimization endpoints are currently unreachable.",
  onRetry,
  className = "",
}) => {
  const { lastApiError, apiBaseUrl, retryConnection, isApiLoading } = useDataSource();
    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-rose-300 leading-relaxed">{description}</p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs text-rose-300/70">
              Ensure <code className="text-rose-200">uvicorn backend.app.main:app</code> is running on port 8000.
            </span>
          </div>
        </div>
      </div>
