...
export const HomeTab: React.FC<HomeTabProps> = ({
  ...
  const handleExternalNavigate = () => {
    // Open Google Maps intent for Nairobi Terminal
    window.open("https://maps.google.com/?q=Kenya+Pipeline+Company+Nairobi+Terminal+PS10", "_blank");
  };

  return (
    <div className="space-y-3.5 pb-2">
      {/* 1. Offline Mode Alert Banner (Shown when offline) */}
      {syncState === "OFFLINE" && (
        <div className="p-3 rounded-lg bg-amber-500/15 border border-amber-500/30 text-amber-200 text-xs flex items-start gap-2 animate-fade-in">
          <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
          <div className="leading-snug">
            <strong className="font-semibold block text-amber-300">OFFLINE MODE</strong>
            Your latest collection data is cached on this device. Updates will synchronize automatically when connectivity is restored.
          </div>
        </div>
      )}

      {/* 2. Safety First Advisory Strip */}
      ...
      {/* 3. Current Collection Card */}
      <div className="p-3.5 rounded-xl bg-[#0F1B2B] border border-[#1C2C42] shadow-sm flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              ...
              <span className={`text-[9px] font-mono px-1.5 py-0.2 rounded border uppercase font-bold ${getStatusBadge(currentStage.collectionStatus)}`}>
                {currentStage.collectionStatus}
              </span>
            </div>
            <div className="text-[11px] text-slate-400">
              Vivo Energy • Order <strong className="font-mono text-slate-300">LO-NBO-8821</strong>
            </div>
          </div>
        </div>
      ...
      {/* 4. NEXT ACTION HERO CARD (DOMINANT visual element on screen) */}
      <div className="p-5 rounded-2xl bg-gradient-to-b from-[#112318] via-[#0D1826] to-[#0A131F] border-2 border-emerald-500/60 shadow-xl space-y-3 relative overflow-hidden">
        ...
        <div>
          ...
          <h1 className="text-xl xs:text-2xl font-black tracking-tight text-white mt-0.5 leading-tight">
        ...
        {/* Primary Acknowledge Button or Confirmed State */}
      ...
      {/* 5. Turnaround Recovery Banner (When FlowGuard Intervention improved timing) */}
      {currentStage.isRecovering && (
        <div className="p-3 rounded-xl bg-emerald-950/70 border border-emerald-500/40 text-emerald-200 text-xs flex items-center justify-between gap-3 animate-fade-in">
          <div className="flex items-center gap-2">
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between rounded-md p-1.5 transition-colors ${
                  isCurrent ? "bg-emerald-500/10 border border-emerald-500/30 text-white font-bold" : "text-slate-400"
                }`}
              >
                <div className="flex items-center gap-2">
                  {isCompleted ? (
                  ...
                  <span className={isCompleted ? "line-through text-slate-500" : isCurrent ? "text-emerald-300" : "text-slate-400"}>
                    {step.title}
                  </span>
                </div>

                <span className="font-mono text-[10px] text-slate-500">
                  {step.timestamp || (isCompleted ? "Done" : isCurrent ? "Current" : "Pending")}
                </span>
              </div>
            );
          })}
      ...
      {/* 7. Predictive Timing Summary (3 Indicators) */}
      <div className="grid grid-cols-3 gap-2">
        {/* Arrival Window */}
        <div className="p-2.5 rounded-xl bg-[#0F1B2B] border border-[#1C2C42] space-y-1">
        ...
        <div className="p-2.5 rounded-xl bg-[#0F1B2B] border border-[#1C2C42] space-y-1">
          ...
          <span className="text-[9px] text-slate-400 block truncate">
            {currentStage.turnaroundNote.split("•")[0]}
          </span>
        </div>

        {/* Gate-Out Window */}
        <div className="p-2.5 rounded-xl bg-[#0F1B2B] border border-emerald-500/30 space-y-1">
      ...
      <div className="grid grid-cols-3 gap-2 pt-1">
        ...
        >
          ...
          <span className="text-[10px] font-bold">Navigate KPC</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] font-bold">All Updates</span>
        </button>
      </div>
