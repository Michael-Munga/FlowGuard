...
import { Home, Truck, Bell, Navigation, MoreHorizontal } from "lucide-react";
...
export const DriverBottomNav: React.FC<DriverBottomNavProps> = ({
  ...
  const tabs: { id: DriverTab; label: string; icon: React.ReactNode; badge?: number }[] = [
    ...
    { id: "COLLECTION", label: "Collection", icon: <Truck className="w-5 h-5" /> },
    {
      id: "UPDATES",
      label: "Updates",
    ...
    { id: "NAVIGATE", label: "Navigate", icon: <Navigation className="w-5 h-5" /> },
  ...
  return (
    <nav
      ...
      className="bg-[#0B1420] border-t border-slate-800 grid grid-cols-5 p-1.5 shrink-0 shadow-2xl"
    >
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`relative py-2 px-0.5 rounded-lg flex flex-col items-center justify-center gap-1 transition-all cursor-pointer select-none ${
              isActive
